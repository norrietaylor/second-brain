...
obsidian vault=second-brain search query="<person-name>"

# Search meeting notes that mention them
obsidian vault=second-brain search query="<person-name>" tags="meeting"
```
...
3. **Search the vault** for existing notes on the topic:
   ```bash
   obsidian vault=second-brain search query="X topic"
   ```
...
```bash
obsidian vault=second-brain create path="04 Data/YYYY/MM/YYYY.MM.DD-<person-name>.md" content="<content>" silent
```
