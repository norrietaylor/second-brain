...
```bash
obsidian vault={{VAULT_NAME}} base:query path="05 Meta/bases/Unprocessed Inbox.base" format=json
```
...
```bash
obsidian vault={{VAULT_NAME}} base:query path="05 Meta/bases/Dirty Notes.base" format=json
```
...
```bash
obsidian vault={{VAULT_NAME}} base:query path="02 Areas/Meetings.base" view="Today" format=json
```
...
4. If it has Log/Action Items content but no Summary:
   ...
   d. Set the frontmatter `summary` property:
      ```bash
      obsidian vault={{VAULT_NAME}} property:set name=summary value="<one-liner>" path="<meeting-path>"
      ```
...
```bash
obsidian vault={{VAULT_NAME}} append path="<DAILY_NOTE_PATH>" content="<digest sections>"
```
...
3. For each **checked** item:
   - Search for the task note with matching `github_url` and `status: pending`:
     ```bash
     obsidian vault={{VAULT_NAME}} search query="github_url: GITHUB_URL" path="04 Data" format=json
     ```
   - Filter results for `type: task` with `status: pending`
   - Set `status: done` via:
     ```bash
     obsidian vault={{VAULT_NAME}} property:set name=status value=done path="TASK_NOTE_PATH"
     ```
...
```bash
obsidian vault={{VAULT_NAME}} append path="<DAILY_NOTE_PATH>" content="<slack activity section>"
```
...
```bash
obsidian vault={{VAULT_NAME}} base:query path="01 Projects/Active Projects.base" format=json
obsidian vault={{VAULT_NAME}} base:query path="02 Areas/People.base" format=json
