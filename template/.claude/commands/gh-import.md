...
```bash
obsidian vault={{VAULT_NAME}} base:query path="02 Areas/GitHub.base" format=json
```
...
```bash
obsidian vault={{VAULT_NAME}} search query="github_url: GITHUB_URL" format=json
```

**Fallback 2 — Alias lookup:**
```bash
obsidian vault={{VAULT_NAME}} read file="gh-REPO-NUMBER"
```
...
```bash
obsidian vault={{VAULT_NAME}} property:set file="ALIAS" property=github_state value="NEW_STATE"
obsidian vault={{VAULT_NAME}} property:set file="ALIAS" property=github_labels value='["label1","label2"]' type=list
obsidian vault={{VAULT_NAME}} property:set file="ALIAS" property=github_assignees value='["user1"]' type=list
obsidian vault={{VAULT_NAME}} property:set file="ALIAS" property=github_last_synced value="NEWEST_ACTIVITY"
```
